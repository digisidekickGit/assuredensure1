...
const generatePDFForPurchase = async (req, res, next) => {
  try {
    const doc = new PDFDocument({
      size: "A4", // Set the page size to A4
    });
    const { tableData, POS, EnterDate, tableSummery } = req;
    const { id } = req.params;

    const POS_NAME = `${POS?.ReportingTo?.Salutation ?? ""} ${
      POS?.ReportingTo?.MiddleName ?? ""
    } ${POS?.ReportingTo?.Name ?? ""} ${POS?.ReportingTo?.LastName ?? ""}`;
    const Formatid = `${POS?.WDAddress} ${POS?.WDLocality} ${POS?.WDCity}
 ${POS?.WDState} - ${POS?.WDPincode}`;

    // 1 = Daily , 7 Days = Weekly, 10 = 10 Days , 15 = 15days , 30 = 1 Monthly

    let posPayOutCycle = "Daily";
    switch (POS.PayoutCycle) {
      case 1:
        posPayOutCycle = "Daily";
        break;
      case 7:
        posPayOutCycle = "Weekly";
        break;
      case 10:
        posPayOutCycle = "10 Days";
        break;
      case 15:
        posPayOutCycle = "15 days";
        break;
      case 30:
        posPayOutCycle = "1 Monthly";
        break;
    }
    ...
    // addText("IPOS Certification Date:", 240, 180, 200);

    addText(POS_NAME, 380, 80, 200, "right");
    // addText("Gaurav Chawla", 380, 100, 200, "right");
    addText(posPayOutCycle, 380, 100, 200, "right");
    addText(EnterDate, 380, 120, 200, "right");
    addText(POS?.GSTINNumber?.toUpperCase() ?? "", 380, 140, 200, "right");
    addText(POS?.PanNumber?.toUpperCase() ?? "", 380, 160, 200, "right");
...
module.exports = { generatePDFForPurchase };
